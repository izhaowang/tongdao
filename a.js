// 初始化ECharts实例
        ...
        chartDom.addEventListener('mousemove', function(e) {
            ...
            if (point2) {
                ...
                // 更新图表与坐标显示
                if (coordinateInfo) coordinateInfo.textContent = `绘制中: (${rectX.toFixed(2)}, ${rectY.toFixed(2)})  ${rectWidth.toFixed(2)} x ${rectHeight.toFixed(2)}`;
                updateChart();
            }
        });
        ...
        chartDom.addEventListener('mouseup', function(e) {
            ...
            // 清除坐标显示（恢复默认）
            if (coordinateInfo) coordinateInfo.textContent = '';
