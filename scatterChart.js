...
const markerOk = document.getElementById('marker-ok');
...
// 确定按钮事件
if (markerOk) {
    markerOk.addEventListener('click', function () {
        const selectedValue = markerSelect.value; // "外部干扰"
        alert(`您标记的内容是：${selectedValue}`); // 可根据需要替换为实际存储操作
        hideMarkerDialog();
    });
}
...
document.addEventListener('click', function (e) {
    if (markerDialog && markerDialog.style.display === 'block' && !markerDialog.contains(e.target)) {
        hideMarkerDialog();
    }
});
// 更新界面显示
...
// 菜单项
const menuItems = [
    { text: '清除标记', action: clearAllRectangles },
    { text: '标记为', action: showMarkerDialog }
];
...
function showContextMenu(x, y) {
    ...
    // if (menuWidth === 0) menuWidth = 100;  // 默认宽度
    // if (menuHeight === 0) menuHeight = 80; // 默认高度
    ...
    console.log(`显示菜单 at (${x}, ${y})`, contextMenu.style.top, contextMenu.style.left);

    // 调试日志（可删除）
    // console.log('菜单尺寸:', menuWidth, menuHeight, '位置:', left, top);
}
...
chartDom.addEventListener('contextmenu', function (e) {
    ...
    // 使用 ECharts 的 containPixel 方法判断该像素点是否在 grid 内
    if (myChart.containPixel('grid', [pixelX, pixelY])) {
        e.preventDefault(); // 阻止浏览器默认右键菜单
        // 隐藏可能已显示的菜单（如果希望每次右键都重新定位，可以先隐藏再显示）
        hideContextMenu();
